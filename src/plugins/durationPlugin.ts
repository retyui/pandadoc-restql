    ...
    requestDidStart: ({ context }) => {
      ...
      return {
        willSendResponse({ response }) {
          ...
          response.extensions.duration =
            hrend[0] + Math.round(hrend[1] / 1000000) / 1000;

          response.extensions.vendorRequestsNum =
            context?.vendorRequests?.length;
          response.extensions.vendorRequests = context?.vendorRequests?.map(
            (d: any) => `${d.duration} ${d.status} ${d.size} ${d.url}`
          );
        },
      };
    },
